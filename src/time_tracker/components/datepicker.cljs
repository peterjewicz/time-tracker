(ns time-tracker.components.datepicker
...
(defn update-passed-store [val store]
  ...
  (println (generate-date-string val))
...
(defn datepicker [store]
  ...
  (let [vals (:end @store)
        currentDate (moment)
        date (atom {:month (.format currentDate "MMMM") :date (.date currentDate) :year (.year currentDate)})
